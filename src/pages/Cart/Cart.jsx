    ...
    <div className={`container ${styles.cartPage}`}>
      ...
      <div className={styles.cartGrid}>
        <div className={styles.cartItems}>
          {items.map(item => (
            <div key={item.id} className={styles.cartItem}>
              ...
              <button onClick={() => removeItem(item.id)} className={styles.removeBtn}><FaTrash /></button>
            </div>
          ))}
        </div>
        <div className={styles.orderSummary}>
          ...
          <div className={`${styles.summaryRow} ${styles.total}`}><strong>Total</strong><strong>{formatCurrency(subtotal)}</strong></div>
          <button onClick={handleCheckout} className={styles.ctaButton}>Finalizar Compra</button>
        </div>
      </div>
    </div>
  );
