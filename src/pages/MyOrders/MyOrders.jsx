...
// --- Constantes ---
const WHATSAPP_NUMBER = "5581999998888"; // Substitua pelo seu número
    ...
    if (error) {
        return (
            <div className="container" style={{ textAlign: 'center', padding: '4rem' }}>
                ...
                <Link to="/" style={{ fontSize: '1.8rem', textDecoration: 'underline' }}>
                    Voltar para a Home
                </Link>
            </div>
        );
        ...
        <div className={`container ${styles.myOrdersPage}`}>
            ...
            {orders.length === 0 ? (
                <div className={styles.noOrders}>
                    ...
                    <p>Que tal explorar nossa loja?</p>
                ...
                <div className={styles.orderList}>
                    {orders.map(order => {
                        ...
                        const trackingCode = order.status === 'enviado' || order.status === 'entregue'
                            ? `BR123456${order.id.substring(4, 8)}BR`
                            : null;
                        ...
                        return (
                            <div key={order.id} className={styles.orderCard}>
                                ...
                                <div className={styles.orderItems}>
                                    {order.itens.map(item => (
                                        <div key={item.id} className={styles.orderItem}>
                                            ...
                                            <Link 
                                                to={`/produto/${item.productId}#avaliacoes`} 
                                            ...
                                            <div className={styles.trackingInfo}>
                                                ...
                                                {trackingCode && (
                                                    <>
                                                        ...
                                                        <p className={styles.trackingCode}>{trackingCode}</p>
                                                    </>
                                                )}
                                            </div>
                                        </div>
                                ...
                                <div className={styles.orderFooter}>
                                    <a 
                                        href={generateWhatsAppLink(order.id)} 
                                        target="_blank" 
                                        rel="noopener noreferrer" 
                                        className={`${styles.footerButton} ${styles.helpButton}`}
                                    >
